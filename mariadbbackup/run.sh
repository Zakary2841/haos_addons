...
if ! TZ="$DB_TIMEZONE" date >/dev/null 2>&1; then
  ...
  log "ERROR" "Check valid zones at https://en.wikipedia.org/wiki/List_of_tz_database_time_zones"
  exit 1
fi

export TZ="$DB_TIMEZONE"

log "INFO" "Starting MariaDB dump process..."
...
if command -v pigz >/dev/null 2>&1; then
    ...
    COMPRESS_CMD="gzip"
    log "INFO" "Using gzip (install pigz in Dockerfile for faster compression)"
fi

# Validate backup directory exists
if [ ! -d "$OUTPUT_FOLDER" ]; then
  log "ERROR" "Backup directory does not exist: $OUTPUT_FOLDER"
  log "ERROR" "Ensure the path is correct and the mount is connected."
  exit 1
fi

# Validate backup directory is writable
if [ ! -w "$OUTPUT_FOLDER" ]; then
  log "ERROR" "Backup directory is not writable: $OUTPUT_FOLDER"
  exit 1
fi

# Generate and validate filename timestamp
TIMESTAMP=$(date +"$DB_FILENAME_FORMAT" 2>&1)
if [ $? -ne 0 ]; then
  log "ERROR" "Invalid filename format: $DB_FILENAME_FORMAT"
  log "ERROR" "Format error: $TIMESTAMP"
  exit 1
fi
...
if [ "$DB_USE_FOLDERS" = "true" ]; then
  if [ ! -d "$ARCHIVE_FOLDER" ]; then
    log "INFO" "Creating archive directory: $ARCHIVE_FOLDER"
...
if [ -z "$DATABASES" ]; then
  log "ERROR" "Could not retrieve database list. Check credentials and connection."
  exit 1
fi
...
if [ "$DB_INCLUDE_SYSTEM" = "true" ] && [ "$DB_SEPARATE_FILES" != "true" ]; then
  ...
  log "INFO" "Backing up ALL databases to single file -> $OUTPUT_FILE"
  ...
  if mariadb-dump --skip-ssl --single-transaction --quick --events --routines --triggers \
    ...
    log "SUCCESS" "Full database backup completed."
  else
...
if [ "$DB_USE_FOLDERS" = "true" ]; then
  ...
  log "INFO" "Moving old backups to $DB_ARCHIVEDIR..."
  ...
  log "INFO" "Cleaning old backups (files older than $DB_RETENTION_DAYS days)..."
